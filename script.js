...
function renderHome() {
  ...
  Object.entries(mantras).forEach(([id, mantra]) => {
    ...
    card.innerHTML = `
      <span class="title">${mantra.title}</span>
...
function renderMantra() {
  ...
  lines.forEach(block => {
    ...
    explicitLines.forEach(line => {
      ...
      //spliting textblovk for both hindi and english
      ...
      if (tempStr.trim()) {
        allSentences.push(tempStr.trim());
      }
    });
  });

  //grouping sentences together for better readebility
  ...
  mantraText.scrollTop = 0;
}

//increment counter for the person to keeptrack of how many jaaps done
function incrementCounter() {
  if (!currentMantraId) return;

  const currentCount = counters[currentMantraId] || 0;
  counters[currentMantraId] = currentCount + 1;
  persistCounters();
  updateCounterUI();
...
function resetCounter() {
  ...
  const ok = window.confirm("Reset this mantra count?");
  if (!ok) return;
...
function setLanguageUI() {
  languageToggle.textContent = language === "hindi" ? "हिंदी" : "Hinglish";
  languageToggle.setAttribute("aria-pressed", String(language === "hindi"));
}
...
function getStoredLanguage() {
  ...
  return savedLanguage === "hinglish" ? "hinglish" : "hindi";
}
